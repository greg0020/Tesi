...
def plot_comparison(drl_metrics: dict, mr_metrics: dict,
                    save_path: str = None):
    ...
    # 2. Drawdown
    ax = axes[0, 1]
    for label, metrics in [('DRL Agent', drl_metrics), ('Mean Reversion', mr_metrics)]:
    ...
    table = ax.table(cellText=table_data,
                     colLabels=['Metric', 'DRL Agent', 'Mean Reversion'],
                     loc='center',
                     cellLoc='center')
...
def main():
    ...
    strategy_type = train_config.get('strategy_type', 'drl')  # default 'drl' per compatibilità
